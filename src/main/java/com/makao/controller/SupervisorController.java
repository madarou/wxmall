...
public class SupervisorController {
	...
	public @ResponseBody Object login(@RequestBody JSONObject paramObject,HttpServletRequest request, HttpServletResponse response)
	{
		...
		Supervisor s = this.supervisorService.queryByName(userName);
		if(s==null){
			jsonObject.put("msg", "202");
			return jsonObject;
		}
		boolean pass = EncryptUtils.passwordEncryptor.checkPassword(password, s.getPassword());
		if(!pass){
			jsonObject.put("msg", "203");
			return jsonObject;
		}
		jsonObject.put("msg", "200");
		TokenModel tm = tokenManager.createToken(s.getId(), "s");
