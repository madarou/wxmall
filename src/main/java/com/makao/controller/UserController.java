			...
			page = "<!DOCTYPE html>"
					+ "<html>"
						+ "<head>"
							...
							+ "<title>社享网</title>"
							+ "<link rel=\"stylesheet\" href=\""+MakaoConstants.CDN_DOMAIN+"/css/index.djs.css\" />"
							+ "<link rel=\"stylesheet\" href=\""+MakaoConstants.CDN_DOMAIN+"/css/animate.css\" />"
							+ "<link rel=\"stylesheet\" href=\""+MakaoConstants.CDN_DOMAIN+"/css/font-awesome.min.css\" />"
						    + "<link rel=\"stylesheet\" href=\""+MakaoConstants.CDN_DOMAIN+"/css/iconfont.css\" />"
							+ "<style>"
								+".flash{position:fixed;top:0;left:0;background-color:#77CFDD !important;width:100%;height:100%;z-index:99999;background:url(\""+MakaoConstants.CDN_DOMAIN+"/img/opening.png\") no-repeat center;"
								+"background-size:27% !important}"									
								+".jumpcss{position: absolute;display: inline-block;right:10%;top:5%;text-align:center;border-radius: 4px;color: white;border: 1px solid white;padding: 0.2em 0.4em;}"
							+"</style>"
						+ "</head>"
						+ "<body>"
							+"<div class=\"flash\" id=\"flash\"><a id=\"jump\" class=\"jumpcss\" onclick=\"disp();\">跳过(2s)</a></div>"
							...
							+ "<script>"
								...
								+ "var head = '"+user.getAvatarUrl()+"';"
								+ "var points = "+user.getPoint()+";"
								+ "var cityid="+cityid+";"
								...
								+"function showTime(count) {"
									+"console.log(count);"
									+"document.getElementById(\"jump\").innerHTML=\"跳过(\"+count+\"s)\";"
									+"if (count >= 0) {"
										+"count -= 1;"
										+"setTimeout(function () {"
											+"showTime(count);"
							...
							+ "<script src=\""+MakaoConstants.CDN_DOMAIN+"/static/bundle.31e11937.js\"></script>"
						+ "</body>"
					+ "</html>";
			out.write(page);
			out.flush();out.close();
						...
						+ "<html>"
							...
							+"<div class=\"flash\" id=\"flash\"><a id=\"jump\" class=\"jumpcss\" onclick=\"disp();\">跳过(2s)</a></div>"
								...
								+ "<script>"
									...
									+ "var IMG_URL = '"+MakaoConstants.SERVER_DOMAIN+"/static/upload/';"
									+ "var user_id = "+u_id+";"
									+ "var name = '"+u.getUserName()+"';"
									+ "var head = '"+u.getAvatarUrl()+"';"
									+ "var points = "+u.getPoint()+";"
