...
public class BannerDaoImpl implements IBannerDao {
	/** 日志实例 */
	private static final Logger logger = Logger.getLogger(BannerDaoImpl.class);
	@Resource
	...
	public Banner getById(int id) {
		// return (Banner) sessionFactory.getCurrentSession().createQuery(
		// "from Banner u where u.id=?").setInteger(0, id)
		// .uniqueResult();
		Session session = null;
		Transaction tx = null;
		Banner res = null;
		try {
			session = sessionFactory.openSession();// 获取和数据库的回话
			tx = session.beginTransaction();// 事务开始
			res = (Banner) session.get(Banner.class, id);
			tx.commit();// 提交事务
		} catch (HibernateException e) {
			if (null != tx)
				tx.rollback();// 回滚
	...
	public List<Banner> queryAll() {
		Session session = null;
		Transaction tx = null;
		List<Banner> res = null;
		try {
			session = sessionFactory.openSession();// 获取和数据库的回话
			tx = session.beginTransaction();// 事务开始
			res = session.createQuery("from Banner").list();
			tx.commit();// 提交事务
		} catch (HibernateException e) {
			if (null != tx)
				tx.rollback();// 回滚
	...
	public List<Banner> queryByName(String name) {
		...
		try {
			session = sessionFactory.openSession();// 获取和数据库的回话
			tx = session.beginTransaction();// 事务开始
			res = session.createQuery("from Banner u where u.bannerName like ?")
	...
	public List<Banner> queryByAreaId(int areaId) {
		...
		try {
			session = sessionFactory.openSession();// 获取和数据库的回话
			tx = session.beginTransaction();// 事务开始
			res = session.createQuery("from Banner b where b.areaId=?")
					.setInteger(0, areaId).list();
			tx.commit();// 提交事务
		} catch (HibernateException e) {
			if (null != tx)
				tx.rollback();// 回滚
