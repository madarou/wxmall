...
public class OrderOffDaoImpl implements IOrderOffDao {
	...
	public List<OrderOff> queryAllCanceledAndReturned(String tableName) {
		String sql = "SELECT * FROM "+ tableName + " WHERE `finalStatus` IN ('"+OrderState.RETURNED.getCode()+"','"+OrderState.CANCELED.getCode()+"')"
				+ " AND `refundStatus` NOT IN ('无需退款','已退款') Order By `finalTime`";
		Session session = null;
		Transaction tx = null;
		List<OrderOff> res = new LinkedList<OrderOff>();
		try {
	...
	public List<OrderOff> queryDealed(String tableName, int areaid,
		...
		String sql = "SELECT * FROM "+ tableName + " WHERE `areaId`="+areaid+" AND `finalStatus` IN ('"+OrderState.TERMINALED.getCode()+"','"+OrderState.RETURN_CANCELED.getCode()+"') "
				+ "AND `ordertime` BETWEEN '"+fromdate+"' AND '"+todate+"' Order By `finalTime`";
		Session session = null;
		Transaction tx = null;
		List<OrderOff> res = new LinkedList<OrderOff>();
		try {
	...
	@Override
	public List<OrderOff> queryReturned(String tableName, int areaid,
			String fromdate, String todate) {
		String sql = "SELECT * FROM "+ tableName + " WHERE `areaId`="+areaid+" AND `finalStatus`='"+OrderState.REFUNDED.getCode()+"' "
				+ "AND `ordertime` BETWEEN '"+fromdate+"' AND '"+todate+"' Order By `finalTime`";
		Session session = null;
		Transaction tx = null;
		List<OrderOff> res = new LinkedList<OrderOff>();
