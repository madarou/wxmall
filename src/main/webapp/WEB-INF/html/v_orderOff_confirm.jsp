 ...
 <ul>
  <li>
   <dl>
    ...
    <dd><a href="/orderOff/v_query_confirm/${id}/1?token=${token}" class="active">已收货订单</a></dd>
 ...
 <div class="rt_content">
     ...
     <section>
      <div class="page_title">
       <b>用户确认收货或系统在一定时间后自动确认收货的订单(注：此状态的订单在一定时间内如果没有被申请退货，会自动进入已完成订单列表)：</b><a class="fr top_rt_btn" href="/orderOff/v_query_confirm/${id}?token=${token}">刷新</a>
      </div>
      <table class="table">
       <tr>
        <th>订单编号</th>
      ...
      <aside class="paging">
       <a href="/orderOff/v_query_confirm/${id}/1?token=${token}">第一页</a>
       <c:forEach var="item" begin="1" end="${pageCount}">
		   <a href="/orderOff/v_query_confirm/${id}/${item}?token=${token}">${item}</a>
	   </c:forEach>
       <a href="/orderOff/v_query_confirm/${id}/${pageCount}?token=${token}">最后一页</a>
      </aside>
     </section>
	</div>
     </section>
