...
<aside class="lt_aside_nav content mCustomScrollbar">
 ...
 <ul>
  ...
  <li>
   ...
   <dl>
    <dt>订单统计</dt>
    <dd><a href="/orderOff/s_querydealed/${id}?token=${token}"  class="active">成交订单统计</a></dd>
     ...
     $(document).ready(function(){
    	  ...
    	  $("#search").click(function(){
    		  ...
    		  $.ajax({
 	    		 type: "POST",
 		          contentType: "application/json",
 		          url: "/orderOff/querydealed/"+$("#loginUserId").val(),
 		          dataType: "json",
 		        	 ...
 		        	 $("#otable tbody").html("");
 		        	  if(data.msg=="200"){
 		        		  if(data.orders==null||data.orders.length==0){
 		        			 $("#totalT").html("0.00元");$("#totalB").html("0.00元");
 		        		 ...
 		        		 $.each(data.orders,function(i, val){
 		        			 ...
 		        			 var tr = '<tr class="tg-'+group+'">'
 		        				        ...
 		        				        +'<td id="productIds-'+val.id+'" style="display:none">'+val.productIds+'</td>'
 		        				        +'<td id="areaId-'+val.id+'" style="display:none">'+val.areaId+'</td>'
 		        				        +'<td id="cityId-'+val.id+'" style="display:none">'+val.cityId+'</td>'
 		        			 		+'</tr>';
 		        			 table.append(tr);
 		        		 });
 		        		...
 		        		var orderId_toView = 0;
 		        	     ...
 		        	     //弹出文本性提示框
 		        	     $(".viewOrder").click(function(){
 		        	    	$("#productList").html('<tr><td colspan="3">购买商品信息</td></tr><tr><td>商品名称</td><td>规格</td><td>单价</td><td>数量</td></tr>');
 		        	       $(".pop_bg").fadeIn();
 		        	       var clickedId = $(this).attr("id");
 		        	       orderId_toView = clickedId.split("-")[1];
 		        	       $("#oname_phone").text($("#receiverName-"+orderId_toView).text()+"  "+$("#phoneNumber-"+orderId_toView).text());
      ...
      <div class="" style="text-align:center">
		      ...
		      <ul class="ulColumn2">
		       <li>
				    ...
				    <form id="exportform" action="/orderOff/export/${id}/?token=${token}" method="post" style="display:inline">
      		...
      		<div style="float:right;font-weight: bold;font-size: 14px;"><span class="item_name">全部金额累计:</span><span class="item_name" id="totalT" style="color:red">0.00元</span></div>
      		<br/>
      		<div style="float:right;font-weight: bold;font-size: 14px;"><span class="item_name">去成本金额累计:</span><span class="item_name" id="totalB" style="color:red">0.00元</span></div>
         </div>
